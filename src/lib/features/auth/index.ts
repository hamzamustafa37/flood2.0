    ...
    (dispatch, getState) => {
        ...
        signUpUser(user as IUserToBeRegister)
            .then(res => {
                ...
                dispatch(endLoading({ key: ApiActions.USER_CREATION }));
...
export const _socialProvider =
    ...
    dispatch => {
        dispatch(startLoading({ key: ApiActions.SOCIAL_LOGIN_GOOGLE }));
        socialProvider(userInfo)
            ...
            .finally(() => {
                dispatch(endLoading({ key: ApiActions.SOCIAL_LOGIN_GOOGLE }));
            });
    };
export const _verifyOtp =
    ...
    dispatch => {
        dispatch(startLoading({ key: ApiActions.VERIFY_OTP }));
        verifyOtp(email, otp)
            .then(res => {
                if (
                    res.data.meta.status === 200 ||
                    ...
                    dispatch(endLoading({ key: ApiActions.VERIFY_OTP }));
                    setCookie('otp', otp ?? '');
                    router.push(pathName);
                }
            })
...
export const _resendOtp =
    ...
    dispatch => {
        dispatch(startLoading({ key: ApiActions.RESEND_OTP }));
        resendOtp(email ?? email)
            .then(res => {
                if (
                    ...
                    dispatch(endLoading({ key: ApiActions.RESEND_OTP }));
                }
            })
            .catch(err => {
                dispatch(endLoading({ key: ApiActions.RESEND_OTP }));
...
export const _getCurrentUser =
    ...
    dispatch => {
        dispatch(startLoading({ key: ApiActions.GET_CURRENT_USER }));
        getCurrentUser()
            .then(user => {
                ...
                dispatch(
                    endLoading({
                        key: ApiActions.GET_CURRENT_USER,
                    })
                );
                if (callBack) {
                    callBack(user);
            ...
            .catch(err => {
                dispatch(endLoading({ key: ApiActions.GET_CURRENT_USER }));
                showError(err);
            });
    };
export const _forgotPassword =
    ...
    dispatch => {
        dispatch(startLoading({ key: ApiActions.FORGOT_PASSWORD }));
        forgotPassword(email)
            .then((): void => {
                dispatch(endLoading({ key: ApiActions.FORGOT_PASSWORD }));
...
export const _resetPassword =
    ...
    dispatch => {
        dispatch(startLoading({ key: ApiActions.RESET_PASSWORD }));
        resetPassword(email, otp, password)
            .then((): void => {
                dispatch(endLoading({ key: ApiActions.RESET_PASSWORD }));
...
export const _editUser =
    ...
    dispatch => {
        dispatch(startLoading({ key: ApiActions.UPDATE_USER }));
        updateUserInfo(user)
            .then(res => {
                dispatch(userProfile({ ...user, ...res.data }));
                dispatch(endLoading({ key: ApiActions.UPDATE_USER }));
                if (callBack) {
                    callBack();
                }
            })
...
export const _updatePassword =
    ...
    dispatch => {
        dispatch(startLoading({ key: ApiActions.UPDATE_PASSWORD }));
        updatePassword(data)
...
export const _deactivateUser =
    ...
    dispatch => {
        dispatch(startLoading({ key: ApiActions.DEACTIVATE_USER }));
        deactivateUser()
            .then(() => {
                dispatch(endLoading({ key: ApiActions.DEACTIVATE_USER }));
                if (callBack) {
                    callBack();
                }
            })
